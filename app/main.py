...
BOT_PROVIDER_OPTIONS = {
    "GLOBAL_POOL": "Global · orígenes de Mesino",
    "PERSONAL:MAYAPROVIDER": "Privado · origen personal",
}
...
def _request_is_no_accounting_main(db: Session, req) -> bool:
    ...
    if instance_name == "grupo02maya" and provider_group_id in _maya_provider_group_ids():
        return True
...
def panel_create_bot(
    ...
    try:
        ...
        r2 = requests.post(
            ...
            json={
                "webhook": {
                    "url": "https://panel.rfc.docifymx.com/webhook/evolution-rfc",
                    "enabled": True,
...
def _internal_bots_for_private_broadcast(db: Session) -> list[dict]:
    """
    Lista mini gestores RFC visibles: estáticos + dinámicos BotControl.
...
def _panel_error_bucket(error_message: str | None, status: str | None = None) -> str:
    ...
    # ==========================================================
    # 1) Sin origen / sesión / configuración
    # ==========================================================

    if "NO_PROVIDER_FOR_SPECIAL_FORMAT" in up:
        return "Formato especial sin origen compatible"

    if "NO_PROVIDER_ENABLED" in up:
        return "Sin origen habilitado / sesión caída"
    ...
    if (
        ...
        return "Sesión/SID del origen caído"

    if "NOT_CONFIGURED" in code or "GROUPS_NOT_CONFIGURED" in code:
        return "Falta configuración del origen/grupo"

    if "DISABLED_BEFORE_PROCESSING" in up:
        return "Origen deshabilitado antes de procesar"
    ...
    # ==========================================================
    # 3) RFC/PDF incorrecto o duplicado
    # ==========================================================

    if "WRONG_ACT_TYPE" in up or "PDF DE OTRO TIPO" in up:
        return "PDF de otro tipo de RFC"
    ...
    if "ACT_TYPE_NOT_ALLOWED" in code:
        return "Tipo de RFC no permitido para origen"
    ...
    # ==========================================================
    # 6) Respuesta web/origen incompleta
    # ==========================================================

    if "NO_PDF" in code or "NO PDF" in up:
        return "Origen no devolvió PDF"

    if "EMPTY_OR_USELESS_HTML" in up:
        return "Respuesta vacía/inútil del origen web"
    ...
    if "FALLBACK_NO_PROVIDER_AVAILABLE" in up:
        return "Falló y no hubo origen de respaldo"

    if "FAILED_FALLBACK_TO_" in up:
        return "Falló origen inicial y pasó a fallback"

    # ==========================================================
    # 8) Errores de conexión externa del origen web
    # ==========================================================
    ...
    if (
        ...
        return "Error de conexión / servidor del origen"
    ...
    if "FAILED" in code or "FAILED" in up:
        return "Falla general del origen/sistema"
    ...
    *,
    ...
    provider_control_raw = (
        q.with_entities(
            ...
            func.sum(case((RFC_erronea_cond, 1), else_=0)).label("RFC_erroneas"),
    ...
    for row in provider_control_raw:
        item = {
            ...
            "RFC_erroneas": int(row.RFC_erroneas or 0),
...
def _query_requests_for_panel(
    ...
    if maya_personal_provider:
        q = q.filter(
            ~(
                (RequestLog.instance_name == "grupo02maya")
...
def _panel_instance_rows(rows: list[RequestLog]) -> list[dict]:
    ...
    for r in rows:
        name = r.instance_name or "grupo02"
        if name not in data:
            data[name] = {
                "instance_name": name,
                "total": 0,
...
def panel_audit_group(
    ...
    show_provider = (instance_name or "").strip() in ("", "grupo02")
    ...
    for r in rows:
        html += f"""
            <tr>
              ...
              <td>{_esc(bot_label(r.instance_name or "grupo02", db))}</td>
    ...
    if rows:
        for r in rows:
            ...
            <tr>
              ...
              <td>{_esc(r.act_type)}</td>
              <td class="{status_class}">{_esc(r.status)}</td>
              <td>{_esc(_group_name_cached(r.source_group_id, group_cache) if (r.instance_name or "grupo02") == "grupo02" else "OCULTO")}</td>
...
def panel_remove_shared_promotion(
    ...
    try:
        _notify_client_groups_main(
            rows,
            "⚠️ *Bolsa RFC desactivada*\n\nLa bolsa RFC compartida de este cliente fue desactivada."
        )
    except Exception as e:
...
def panel_apply_shared_promotion(
    ...
    try:
        ...
        _notify_client_groups_main(
            rows,
            (
                f"✅ *Bolsa RFC activada*\n\n"
                f"Tu bolsa RFC *{promo_label}* ya fue activada correctamente.\n"
                f"Tipo: *{tipo_label}*\n"
                f"Cuentas con *{available} RFC disponibles*.\n\n"
                f"Este saldo aplica para todos los grupos asociados a esta bolsa RFC compartida.\n"
    ...
    return {
        "ok": True,
        "message": "Bolsa RFC compartida aplicada correctamente",
        "client_key": client_key,
        "shared_key": shared_key,
        "total_actas": total_actas,
        "is_credit": is_credit,
...
def panel_edit_promotion(
    ...
    if "total_actas" in payload:
        ...
        if total_actas < used_now:
            return {
                "ok": False,
                "error": "TOTAL_RFC_LESS_THAN_USED",
                "used_actas": used_now,
                "total_actas": total_actas,
            }
    ...
    return {
        "ok": True,
        "message": "Bolsa RFC editada sin reiniciar consumos",
...
def panel_recharge_promotion(
    ...
    try:
        add_RFC = int(payload.get("add_RFC") or 0)
    except Exception:
        add_RFC = 0
    ...
    if add_RFC <= 0:
        return {"ok": False, "error": "ADD_RFC_INVALID"}
    ...
    return {
        "ok": True,
        "message": "Bolsa RFC recargada sin reiniciar consumos",
        "group_jid": row.group_jid,
...
def panel_add_group_to_shared_promotion(
    ...
    # Buscar una bolsa RFC activa existente de esa bolsa compartida
    leader = (
        db.query(GroupPromotion)
        .filter(
    ...
    try:
        ...
        send_group_text(
            ...
            (
                ...
                f"Tu grupo fue agregado correctamente a la bolsa RFC *{promo_label}*.\n"
                f"Tipo: *{tipo_label}*\n"
                f"Bolsa compartida: *{shared_key}*\n"
                f"Saldo disponible actual: *{available} RFC*."
            )
        )
    except Exception as notify_exc:
      ...
      <div class="wrap">
        ...
        <div class="section">
          <div class="section-head">
            ...
            <div class="section-sub">RFC por paquetes pagados.</div>
          </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Cliente</th>
                  <th>Bolsa RFC</th>
                  <th class="right">RFC autorizadas</th>
                  <th class="right">RFC consumidas</th>
        ...
        <div class="section">
          <div class="section-head">
            ...
            <div class="section-sub">RFC autorizadas a crédito.</div>
          </div>
...
def remove_group_from_shared_promotion(
    ...
    promo_name = row.promo_name or "Bolsa RFC compartida"
    ...
    try:
        msg = f"""
📦 *Actualización de bolsa RFC*
Este grupo fue retirado de la *bolsa compartida*:

🏷 Bolsa RFC: *{promo_name}*
...
async def panel_save_group_RFC_price(
    ...
    try:
        ...
        price_raw = str(data.get("RFC_price", "")).strip()
        ...
        _set_group_RFC_price(db, group_jid, price)
    ...
    html += f"""
      ...
      <script>
          async function saveRFCPrice(groupJid) {{
            ...
            if (!price) {{
              alert("Ingresa el precio del RFC");
              return;
            }}

            try {{
              const res = await fetch(`/panel/group/${{encodeURIComponent(groupJid)}}/RFC-price`, {{
                method: "POST",
                headers: {{
                  "Content-Type": "application/json"
                }},
          ...
          async function savePromotion(groupJid) {{
            ...
            try {{
              ...
              if (data.ok) {{
                alert(data.message || "Bolsa RFC guardada");
                location.reload();
              }} else {{
                alert(data.error || "Error guardando bolsa RFC");
              }}
            }} catch (e) {{
              alert("No se pudo conectar con el servidor");
            }}
          ...
          async function removePromotion(groupJid) {{
            ...
            try {{
              ...
              if (data.ok) {{
                alert(data.message || "Bolsa RFC desactivada");
                location.reload();
              }} else {{
                alert(data.error || "Error quitando bolsa RFC");
              }}
            }} catch (e) {{
              alert("No se pudo conectar con el servidor");
            }}
...
⚡ *RFC SUPER RÁPIDAS SALIENDO EN SEGUNDOS*
...
BROADCAST_CERRADO_MSG = """⚡⚡⚡ *SISTEMA DE RFC CERRADO* ⚡⚡⚡
...
def panel_api_RFC(
    ...
    for r in rows[:100]:
        latest.append({
            ...
            "origen": r.provider_name,
            "origen_grupo": r.provider_group_id,
            "mensaje_origen": r.provider_message,
            "pdf_url": r.pdf_url,
...
def panel_resend_request_pdf(
    ...
    instance = (req.instance_name or MAIN_PANEL_INSTANCE or "grupo02").strip()
    ...
    try:
        ...
        caption = "📄 Reenvío de RFC generada previamente."
...
async def panel_bot_set_promo(token: str, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        if total_actas < MIN_BOT_PROMO_RFC:
            return {"ok": False, "error": f"La bolsa RFC mínima es de {MIN_BOT_PROMO_RFC} RFC"}
        ...
        return {
            "ok": True,
            "message": "Bolsa RFC activada y notificada al grupo",
            "group_jid": group_jid,
            "promo_name": promo_name,
            "total_actas": total_actas,
            "used_actas": int(row.used_actas or 0),
...
def botpanel_remove_promotion(
    ...
    try:
        ...
        if not promo:
            db.commit()
            _clear_panel_cache()
            return {"ok": True, "message": "Este grupo no tenía bolsa RFC"}
        ...
        try:
            send_group_text(
                group_jid,
                (
                    "⚠️ *Bolsa RFC retirada*\n\n"
                    "La bolsa RFC de este grupo fue retirada correctamente.\n"
                    "Para continuar con paquete promocional, contRFC al administrador."
                ),
                instance_name=instance_name,
            )
        except Exception as notify_exc:
        ...
        return {
            "ok": True,
            "message": "Bolsa RFC quitada correctamente"
        }
...
def botpanel_provider_mode_ui(token: str, db: Session = Depends(get_db)):
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1">
      <title>Modo de origen - {_esc(title)}</title>
      <style>
        body {{
          font-family: Arial, sans-serif;
          background:#f4f6f8;
    ...
    <body>
      <div class="box">
        ...
        <div class="help">
          <strong>Global:</strong> usa orígenes del sistema principal y sí cuenta.<br>
          <strong>Privado:</strong> usa origen personal y no cuenta en panel, promociones ni límite.
        </div>
      </div>
      ...
      async function saveProviderMode() {{
        ...
        if (!confirm("¿Guardar este modo de origen?")) {{
          return;
        }}
...
@app.get("/gestoria-maya/orígenes", response_class=HTMLResponse)
...
def panel_bot(token: str, db: Session = Depends(get_db)):
    ...
    if bot_admin_blocked:
        ...
        bot_status_label = "APAGADO"
        bot_status_badge = '<span class="badge badge-danger">BOT APAGADO</span>'
    else:
      ...
      <div class="wrap">
        ...
        <div class="box">
          <div class="head">
            ...
            <span class="small">Bloquea, renombra y asigna promociones (mínimo 10 RFC).</span>
          </div>
          ...
          <div class="wrap">
            ...
            <div class="hero">
              ...
              <div class="toolbar">
                ...
                <a href="/panel?token={_esc(settings.ADMIN_PANEL_TOKEN)}&view=prev_month&group_mode={_esc(group_mode)}" class="tool-link {'tool-link-active' if view == 'prev_month' else ''}">Mes anterior</a>
...
<div class="broadcast-block-title">Mensaje privado a mini gestores RFC</div>
        ...
        <div class="box">
          <div class="head collapsible-head open" onclick="toggleSection('promoCompartidaBody', this)">
            <div>
              <strong>Bolsa RFC compartida</strong>
              <span class="small">
                Permite asignar un paquete de RFC a varios grupos para compartir el mismo saldo.
              </span>
            </div>
        ...
        html += """
        <div id="rfcOwnerStatusBox">
          <div class="box">
            <div class="head">
              <strong>💳 Saldo RFC del panel</strong>
              <span class="small">Cargando saldo...</span>
            </div>
          </div>
        </div>
        ...
        (function() {
          try {
            ...
            fetch("/panel/rfc-owner-status-fragment" + qs)
              .then(function(r) { return r.text(); })
              .then(function(html) {
                const box = document.getElementById("rfcOwnerStatusBox");
                if (box) box.innerHTML = html;
              })
              .catch(function(e) {
                console.error("RFC_OWNER_STATUS_ERROR", e);
              });
          } catch(e) {
            console.error("RFC_OWNER_STATUS_MAIN_ERROR", e);
          }
        })();
        </script>
        """
        ...
        html += """
        <style>
        /* RFC_COMMUNICATION_TOOLS_HORIZONTAL_FINAL */
        #rfcCommunicationToolsFinal {
          margin-top: 18px;
          width: 100%;
        }

        #rfcCommunicationToolsFinal .comm-grid {
          display: grid;
        ...
        #rfcCommunicationToolsFinal .comm-col {
          background: rgba(255,255,255,.08);
          border: 1px solid rgba(255,255,255,.14);
          border-radius: 18px;
          padding: 16px;
        ...
        #rfcCommunicationToolsFinal .comm-title {
          font-weight: 900;
          font-size: 1.05rem;
          color: #fff;
          margin-bottom: 14px;
        ...
        #rfcCommunicationToolsFinal textarea {
          width: 100% !important;
          min-height: 115px !important;
          box-sizing: border-box;
        }

        #rfcCommunicationToolsFinal input,
        #rfcCommunicationToolsFinal select,
        #rfcCommunicationToolsFinal button {
          max-width: 100%;
        }

        #rfcCommunicationToolsFinal .box,
        #rfcCommunicationToolsFinal .card {
        ...
        <div id="rfcCommunicationToolsFinal" class="box">
          <div class="head">
            <strong>📣 Herramientas de comunicación</strong>
            <span class="small">Mensajes masivos a la izquierda y privados a mini gestores a la derecha.</span>
          </div>

          <div class="comm-grid">
            <div class="comm-col">
              <div class="comm-title">Mensajes masivos</div>
              <div id="rfcMassMessagesFinalMount"></div>
            </div>

            <div class="comm-col">
              <div class="comm-title">Mensaje privado a mini gestores RFC</div>
              <div id="rfcPrivateMiniManagersFinalMount"></div>
            </div>
          </div>
        </div>
        ...
        (function() {
          function findTitle(text) {
            const nodes = Array.from(document.querySelectorAll("h1,h2,h3,h4,strong,div,span,label"));
            return nodes.find(function(el) {
              return (el.textContent || "").trim().includes(text);
            });
          }

          function bestContainer(titleNode) {
            ...
            for (let i = 0; i < 12 && cur; i++) {
              ...
              if (
                cur.id !== "rfcCommunicationToolsFinal"
                && !cur.closest("#rfcCommunicationToolsFinal")
                && txt.length > 180
                && (
                  cls.includes("box") ||
                  cls.includes("card") ||
          ...
          function moveOne(titleText, mountId) {
            ...
            const block = bestContainer(title);
            if (!block) return false;

            if (block.closest("#rfcCommunicationToolsFinal")) return true;
          ...
          function run() {
            try {
              moveOne("Mensajes masivos", "rfcMassMessagesFinalMount");
              moveOne("Mensaje privado a mini gestores RFC", "rfcPrivateMiniManagersFinalMount");
            } catch(e) {
              console.error("RFC_COMMUNICATION_TOOLS_HORIZONTAL_ERROR", e);
            }
          }
        ...
        html += """
        <script>
        // RFC_BOT_CONTROL_BUTTONS_V2
        window.rfcBotUpdate = function(params) {
          try {
            const pageQs = new URLSearchParams(window.location.search || "");
            const token = pageQs.get("token") || "docifymx2026";
            ...
            .then(function() {
              return fetch("/panel/rfc-bot-control-fragment?token=" + encodeURIComponent(token) + "&v=" + Date.now());
            })
            .then(function(r) {
              return r.text();
            })
            ...
            .catch(function(e) {
              alert("Error actualizando bot: " + e.message);
              console.error("RFC_BOT_UPDATE_ERROR", e);
            });
          } catch(e) {
            alert("Error JS actualizando bot: " + e.message);
            console.error("RFC_BOT_UPDATE_MAIN_ERROR", e);
          }
        };
        ...
        window.rfcBotReset = function(inst, family) {
          if (!confirm("¿Resetear usados de " + family.toUpperCase() + " para " + inst + "?")) {
            return;
          }
        ...
        window.rfcBotBlock = function(inst, action) {
          window.rfcBotUpdate({
            instance: inst,
            action: action,
            family: "all",
        ...
        html += """
        <div id="rfcBotControlNewMount">
          <div class="box">
            <div class="head">
              <strong>Control por bot</strong>
              <span class="small">Cargando límites CLON / IDCIF...</span>
            </div>
          </div>
        </div>
        ...
        (function() {
          try {
            ...
            fetch("/panel/rfc-bot-control-fragment" + qs)
              ...
              .catch(function(e) {
                console.error("RFC_BOT_CONTROL_FRAGMENT_ERROR", e);
              });

            // Ocultar el Control por bot viejo, dejando visible el nuevo.
            setTimeout(function() {
              document.querySelectorAll(".box .head strong").forEach(function(el) {
                const txt = (el.textContent || "").trim();
                if (txt === "Control por bot") {
                  const box = el.closest(".box");
                  if (box && !box.closest("#rfcBotControlNewMount")) {
            ...
            console.error("RFC_BOT_CONTROL_MAIN_ERROR", e);
          }
        })();
        </script>
        """
        ...
        for r in by_instance:
            ...
            inst_js = _esc(inst)
            bot_action_btn = (
                f"""<button class="btn btn-success" onclick="unblockBot('{inst_js}')">Desbloquear</button>"""
                if bot_blocked else
        ...
        html += """
        <div class="box">
          <div class="head"><strong>Resumen por tipo de RFC</strong></div>
          <div class="table-wrap">
            <table>
              <thead>
                <tr>
        ...
        <div class="box">
          ...
          <div class="group-mode-bar">
            <a class="group-mode-link {'group-mode-link-active' if group_mode == 'all' else ''}"
               href="/panel?token={_esc(settings.ADMIN_PANEL_TOKEN)}&view={_esc(view)}&group_mode=all&group_jid={_esc(group_jid)}&provider_name={_esc(provider_name)}&status={_esc(status)}&act_type={_esc(act_type)}&date_from={_esc(date_from)}&date_to={_esc(date_to)}">
              Ver todos los grupos
            </a>
            <a class="group-mode-link {'group-mode-link-active' if group_mode == 'active' else ''}"
               href="/panel?token={_esc(settings.ADMIN_PANEL_TOKEN)}&view={_esc(view)}&group_mode=active&group_jid={_esc(group_jid)}&provider_name={_esc(provider_name)}&status={_esc(status)}&act_type={_esc(act_type)}&date_from={_esc(date_from)}&date_to={_esc(date_to)}">
        ...
        <div class="box">
          <div class="head collapsible-head open" onclick="toggleSection('grupoClienteBody', this)">
            <div>
              ...
              <span class="small">
                Consulta el rendimiento y estado de las solicitudes por origen.
              </span>
            </div>
            <span class="collapse-icon">▼</span>
          </div>
        ...
        if latest:
            for r in latest:
                ...
                <tr>
                  ...
                  <td class="{status_class}">{_esc(r.status)}</td>
                  <td>{_esc(_group_name_cached(r.source_group_id, group_cache) if (r.instance_name or "grupo02") == "grupo02" else "OCULTO")}</td>
                  <td>{_esc(bot_labels_map.get(r.instance_name or "grupo02") or (r.instance_name or "grupo02"))}</td>
                  <td>{_esc(_provider_label(r.provider_name))}</td>
                  <td>{_esc(_fmt_dt(r.created_at))}</td>
                  <td>{_esc(_fmt_dt(r.updated_at))}</td>
                  <td class="small">{_esc(r.error_message)}</td>
...
@app.get("/panel/auditoria-orígenes", response_class=HTMLResponse)
def panel_auditoria_orígenes(
    request: Request,
    view: str = "day",
    group_jid: str = "",
    provider_name: str = "",
    ...
    try:
        ...
        def _audit_link(new_view: str):
            ...
            return "/panel/auditoria-orígenes?" + urlencode(params)

        html = f"""
        <html>
        <head>
          <meta charset="utf-8">
          <title>Auditoría de orígenes</title>
          <style>
            body {{
              font-family: Arial, sans-serif;
              background:#f3f4f6;
        ...
        print("panel_auditoria_orígenes error:", repr(e), flush=True)
        return HTMLResponse(
            content=f"<pre>Error en /panel/auditoria-orígenes: {str(e)}</pre>",
            status_code=500,
        )
...
def test_provider3_session(
    ...
    tipo_RFC = (payload.get("tipo_RFC") or "nacimiento").strip().lower()
...
def _ensure_api_panel_group(db: Session, client: ApiClient):
    ...
    if not row:
        row = AuthorizedGroup(
            ...
            owner_instance=client.panel_instance_name or "grupo02",
            is_hidden=False,
            hidden_in_main=False,
        )
        db.add(row)
    else:
        row.owner_instance = client.panel_instance_name or "grupo02"
        row.is_hidden = False
    ...
    if not alias:
        ...
        alias.owner_instance = client.panel_instance_name or "grupo02"
...
API_ERROR_MESSAGES = {
    ...
    "CURP_INVALID_OR_NOT_FOUND": "La CURP esta mal escrita, no existe o no fue posible encontrar informacion para generar el RFC.",
    "ACTA_NOT_FOUND": "No se encontro un RFC disponible o registrada para el dato solicitado.",
    "NO_PDF_AVAILABLE": "El origen no devolvio un PDF disponible.",
    "WRONG_PDF_DATA": "El PDF recibido no corresponde al dato solicitado.",
    "WRONG_ACT_TYPE": "El PDF recibido corresponde a otro tipo de RFC.",
    "PROVIDER_TIMEOUT": "El origen tardo demasiado en responder.",
    "PROVIDER_FAILED": "El origen no pudo procesar la solicitud.",
    "NO_PROVIDER_AVAILABLE": "No hay origen disponible para procesar esta solicitud.",
...
def _api_clean_internal_error_code(code: str | None) -> str:
    ...
    # Quita prefijo interno de origen privado
...
def _api_public_error_code(raw_error: str | None) -> str:
    ...
    # Deteccion por texto crudo de errores comunes que vienen del origen
    if (
    ...
    # RFC no registrada / no disponible
    if code in {
        "NO_ACTA",
        "ACTA_NOT_FOUND",
...
def api_v1_create_RFC(
    ...
    row = RequestLog(
        ...
        instance_name=client.panel_instance_name or "grupo02",
    ...
    _enqueue_process_request(row, "api_v1_create_RFC")
...
def api_v1_get_RFC(
    ...
    if row.status == "DONE" and row.api_result_base64:
        data["pdf_url"] = str(request.base_url).rstrip("/") + f"/api/v1/RFC/{row.id}/pdf"
...
@app.get("/api/v1/RFC/{request_id}/pdf")
def api_v1_get_RFC_pdf(
    request_id: int,
    authorization: str | None = Header(default=None),
    db: Session = Depends(get_db),
):
...
def api_admin_create_client(
    ...
    panel_instance = (payload.get("panel_instance_name") or "grupo02").strip()
...
def _deliver_text_result(req: RequestLog, text: str, instance_name: str = None):
    instance = req.instance_name or instance_name or "grupo02"
...
def _extract_provider_no_record_identifiers(text_body: str | None) -> list[str]:
    ...
    # Usamos exRFCmente las frases configuradas en PROVIDER_NO_RECORD_TEXT.
    patterns = _provider_no_record_patterns()
...
def _provider_negative_response_info(text_body: str | None) -> dict:
    """
    Detecta respuestas negativas de orígenes WhatsApp en 2 formas:
    ...
    long_negative_patterns = [
        ...
        r"\bRFCMENTO\s+NO\s+ENCONTRADO\b",
...
def _close_provider_negative_response(
    ...
    *,
    ...
    """
    Cierra solicitudes PROCESSING/QUEUED cuando origen WhatsApp responde negativo.
    ...
    # 1) Forma reply/cita: origen responde "SIN", "-", "No hay registros..."
    if quoted_msg_id:
        rows = (
            db.query(RequestLog)
            .filter(
...
def _provider_pdf_match_status_filter():
    # Antes estaba en 5 minutos. Era muy poco para PDFs de orígenes WhatsApp.
    # Si el origen manda el PDF después del timeout, aún debe poder recuperarse.
...
def _pick_matching_processing_req_for_pdf(
    ...
    # El PDF del origen global llega por grupo02,
    # pero el request original puede pertenecer a grupo02moon/max/rywya.
...
def panel_set_group_promotion(
    ...
    return {
        "ok": True,
        "message": "Bolsa RFC guardada correctamente",
        "group_jid": group_jid,
...
def panel_remove_group_promotion(
    ...
    return {
        "ok": True,
        "message": "Bolsa RFC desactivada correctamente",
        "group_jid": group_jid,
    }
...
def is_legacy_known_group(db: Session, group_jid: str) -> bool:
    try:
        ...
        # 4) Si tiene bolsa RFC asociada, también ya es conocido
        promo = (
    ...
    try:
            ...
            if doc:
                ...
                if not is_chain_req:
                    ...
                    if term_status == "UNCERTAIN":
                        ...
                        # En este caso sí se permite respaldo por filename/texto del origen.
        ...
        if created_any:
            ...
            if not bot_name:
                ...
                bot_name = bot_row.label if bot_row and bot_row.label else "🚀 RFC GRUPO02"
...
# =========================
# RFC GRUPO02 WEBHOOK SIMPLE
# =========================
try:
    from app.rfc_webhook import router as rfc_webhook_router
    app.include_router(rfc_webhook_router)
    print("RFC_WEBHOOK_ROUTER_LOADED", flush=True)
except Exception as e:
    print("RFC_WEBHOOK_ROUTER_LOAD_ERROR =", repr(e), flush=True)


@app.get("/panel/rfc-stats-fragment", response_class=HTMLResponse)
def panel_rfc_stats_fragment(request: Request):
    if not _is_valid_admin_panel_token(request):
        return HTMLResponse("No autorizado", status_code=403)

    try:
        day = (request.query_params.get("day") or _panel_day_str()).strip()
        pattern = f"panel_stats:{day}:group:*"

        rows = []

        for raw_key in redis_conn.scan_iter(pattern):
            ...
            group_name = item.get("group_name") or group_jid

            rows.append({
                "group_jid": group_jid,
                "group_name": group_name,
                "total": int(item.get("total") or 0),
                "ok_curp": int(item.get("ok_curp") or 0),
                "ok_rfc_clon": int(item.get("ok_rfc_clon") or 0),
                "ok_rfc_idcif": int(item.get("ok_rfc_idcif") or 0),
                "ok_rfc_idcif_qr": int(item.get("ok_rfc_idcif_qr") or 0),
                "ok_qr": int(item.get("ok_qr") or 0),
                "updated_at": item.get("updated_at") or "",
            })

        rows.sort(key=lambda x: (-x["total"], x["group_name"]))

        if not rows:
            return HTMLResponse("""
            <div class="muted">Sin stats RFC en Redis para hoy.</div>
            """)
        ...
        for r in rows:
            html += f"""
              <tr>
                <td>
                  <strong>{_esc(r["group_name"])}</strong><br>
                  <small>{_esc(r["group_jid"])}</small>
                </td>
                <td><strong>{r["total"]}</strong></td>
                <td>{r["ok_curp"]}</td>
                <td>{r["ok_rfc_clon"]}</td>
                <td>{r["ok_rfc_idcif"]}</td>
                <td>{r["ok_qr"]}</td>
                <td>{r["ok_rfc_idcif_qr"]}</td>
                <td><small>{_esc(r["updated_at"])}</small></td>
              </tr>
            """
        ...
        print("panel_rfc_stats_fragment error:", repr(e), flush=True)
        return HTMLResponse(f"<pre>Error stats RFC: {_esc(str(e))}</pre>", status_code=500)


@app.get("/panel/rfc-stats", response_class=HTMLResponse)
def panel_rfc_stats_clean(request: Request):
    if not _is_valid_admin_panel_token(request):
        return HTMLResponse("No autorizado", status_code=403)
    ...
    try:
        ...
        for raw_key in redis_conn.scan_iter(pattern):
            ...
            row = {
                "group_jid": item.get("group_jid", ""),
                "group_name": item.get("group_name") or item.get("group_jid", ""),
                "total": int(item.get("total") or 0),
                "ok_curp": int(item.get("ok_curp") or 0),
                "ok_rfc_clon": int(item.get("ok_rfc_clon") or 0),
                "ok_rfc_idcif": int(item.get("ok_rfc_idcif") or 0),
        ...
        groups.sort(key=lambda x: (-x["total"], x["group_name"]))
        ...
        if groups:
            ...
            rows_html = """
            <tr>
              <td colspan="8">Sin stats RFC para este día.</td>
            </tr>
            """
        ...
        <body>
          <div class="wrap">
            <div class="top">
              <div>
                <h1>📊 Stats RFC Grupo02</h1>
                <small>Día: {_esc(day)}</small>
              </div>
              <a class="btn" href="/panel?token={_esc(request.query_params.get("token") or "")}">Volver al panel</a>
            </div>

            <div class="cards">
              <div class="card"><div class="label">Total</div><div class="num">{totals["total"]}</div></div>
              <div class="card"><div class="label">CURP</div><div class="num">{totals["ok_curp"]}</div></div>
              <div class="card"><div class="label">RFC</div><div class="num">{totals["ok_rfc_clon"]}</div></div>
              <div class="card"><div class="label">RFC+IDCIF</div><div class="num">{totals["ok_rfc_idcif"]}</div></div>
              <div class="card"><div class="label">QR</div><div class="num">{totals["ok_qr"]}</div></div>
              <div class="card"><div class="label">RFC+IDCIF QR</div><div class="num">{totals["ok_rfc_idcif_qr"]}</div></div>
            </div>
        ...
        print("panel_rfc_stats_clean error:", repr(e), flush=True)
        return HTMLResponse(f"<pre>Error stats RFC: {_esc(str(e))}</pre>", status_code=500)


@app.get("/panel/rfc-bolsas-fragment", response_class=HTMLResponse)
def panel_rfc_bolsas_fragment(request: Request):
    if not _is_valid_admin_panel_token(request):
        return HTMLResponse("No autorizado", status_code=403)
    ...
    try:
        ...
        with engine.begin() as conn:
            # Sincronizar grupos autorizados hacia planes
            conn.execute(text("""
                INSERT INTO rfc_group_plans (
                    group_jid,
                    instance_name,
                    group_name,
                    clon_balance,
                ...
                SELECT
                    group_jid,
                    COALESCE(owner_instance, 'grupo02'),
                    COALESCE(group_name, group_jid),
                    0,
                    1.50,
                    FALSE,
                    500.00
                FROM authorized_groups
                ON CONFLICT (group_jid, instance_name)
                DO NOTHING
            """))
            ...
            rows = conn.execute(text("""
                SELECT
                    group_jid,
                    instance_name,
                    COALESCE(group_name, group_jid) AS group_name,
                    clon_enabled,
                    clon_balance,
                    clon_used,
                    clon_price,
        ...
        for r in rows:
            ...
            idcif_status = "INACTIVO"

            if r["idcif_enabled"] and expires:
                exp = expires
                ...
                if exp > now:
                    ...
                    idcif_status = "VENCIDO"

            expires_txt = ""
            if expires:
                try:
                    expires_txt = expires.strftime("%Y-%m-%d %H:%M")
                except Exception:
        ...
        print("panel_rfc_bolsas_fragment error:", repr(e), flush=True)
        return HTMLResponse(f"<pre>Error rfc-bolsas: {_esc(str(e))}</pre>", status_code=500)


@app.get("/panel/rfc-plan/recargar-clon", response_class=HTMLResponse)
def panel_rfc_plan_recargar_clon(request: Request):
    return HTMLResponse("Acción no permitida desde el panel del gestor.", status_code=403)


@app.get("/panel/rfc-plan/renovar-idcif", response_class=HTMLResponse)
def panel_rfc_plan_renovar_idcif(request: Request):
    return HTMLResponse("Acción no permitida desde el panel del gestor.", status_code=403)


@app.get("/panel/rfc-bolsas-readonly-fragment", response_class=HTMLResponse)
def panel_rfc_bolsas_readonly_fragment(request: Request):
    if not _is_valid_admin_panel_token(request):
        return HTMLResponse("No autorizado", status_code=403)
    ...
    try:
        ...
        print("panel_rfc_bolsas_readonly_fragment error:", repr(e), flush=True)
        return HTMLResponse(f"<pre>Error rfc-bolsas-readonly: {_esc(str(e))}</pre>", status_code=500)


@app.get("/panel/rfc-owner-wallet-fragment", response_class=HTMLResponse)
def panel_rfc_owner_wallet_fragment(request: Request):
    if not _is_valid_admin_panel_token(request):
        return HTMLResponse("No autorizado", status_code=403)
    ...
    try:
        ...
        token = request.query_params.get("token") or ""
        owner = request.query_params.get("owner_instance") or "grupo02"
        ...
        if not wallet:
            return HTMLResponse("""
            <section class="card" style="margin:16px 0;">
              <h3>💼 Bolsa madre RFC</h3>
              <div>No existe bolsa madre para este owner.</div>
            </section>
            """)

        now = datetime.now(timezone.utc)

        exp = wallet["idcif_expires_at"]
        idcif_status = "INACTIVO"

        if wallet["idcif_enabled"] and exp:
            if exp.tzinfo is None:
                exp = exp.replace(tzinfo=timezone.utc)
        ...
        expires_txt = "-"
        if wallet["idcif_expires_at"]:
            try:
                expires_txt = wallet["idcif_expires_at"].strftime("%Y-%m-%d %H:%M")
            except Exception:
                expires_txt = str(wallet["idcif_expires_at"])
        ...
        for g in groups:
            group_options += f"""
              <option value="{_esc(g["group_jid"])}">{_esc(g["group_name"])} - {_esc(g["group_jid"])}</option>
            """

            g_exp = g["idcif_expires_at"]
            g_exp_txt = "-"
            if g_exp:
                try:
                    g_exp_txt = g_exp.strftime("%Y-%m-%d %H:%M")
                except Exception:
                    g_exp_txt = str(g_exp)
            ...
            <tr>
              <td>
                <b>{_esc(g["group_name"])}</b><br>
                <small>{_esc(g["group_jid"])}</small>
              </td>
              <td><b>{int(g["clon_balance"] or 0)}</b></td>
              <td>{int(g["clon_used"] or 0)}</td>
              <td>{'ACTIVO' if g["idcif_enabled"] else 'INACTIVO'}</td>
              <td>{_esc(g_exp_txt)}</td>
              <td>{int(g["idcif_used"] or 0)}</td>
            </tr>
            """

        if not group_options:
            group_options = '<option value="">Sin grupos</option>'

        if not group_rows:
            group_rows = '<tr><td colspan="6">Sin grupos internos.</td></tr>'

        html = f"""
        <section class="card" style="margin:16px 0;">
          <h3 style="margin:0 0 6px;">💼 Bolsa madre RFC del gestor</h3>
          <small>Esta bolsa la recarga el proveedor. El gestor solo reparte CLON a sus grupos.</small>

          <div style="display:grid;grid-template-columns:repeat(4,minmax(120px,1fr));gap:12px;margin-top:12px;">
            <div class="card" style="margin:0;padding:12px;">
              <div><small>CLON disponibles madre</small></div>
              <b style="font-size:24px;">{int(wallet["clon_balance"] or 0)}</b>
            </div>
            <div class="card" style="margin:0;padding:12px;">
              <div><small>CLON usados madre</small></div>
              <b style="font-size:24px;">{int(wallet["clon_used"] or 0)}</b>
            </div>
            <div class="card" style="margin:0;padding:12px;">
              <div><small>IDCIF semanal</small></div>
              <b style="font-size:24px;">{_esc(idcif_status)}</b>
            </div>
            <div class="card" style="margin:0;padding:12px;">
              <div><small>Vence IDCIF</small></div>
              <b>{_esc(expires_txt)}</b>
            </div>
          </div>

          <div style="margin-top:16px;padding:12px;border:1px solid #e5e7eb;border-radius:12px;">
            <h4 style="margin:0 0 10px;">Asignar RFC CLON a grupo</h4>

            <form method="get" action="/panel/rfc-owner/asignar-clon" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
              <input type="hidden" name="token" value="{_esc(token)}">
              <input type="hidden" name="owner_instance" value="{_esc(owner)}">

              <select name="group_jid" style="min-width:340px;padding:9px;border-radius:8px;border:1px solid #d1d5db;">
                {group_options}
              </select>

              <input name="qty" type="number" min="1" placeholder="Cantidad" style="width:120px;padding:9px;border-radius:8px;border:1px solid #d1d5db;">

              <button type="submit" style="background:#14532d;color:white;border:0;border-radius:8px;padding:10px 14px;font-weight:700;">
                Asignar CLON
              </button>
            </form>
          </div>
        ...
        print("panel_rfc_owner_wallet_fragment error:", repr(e), flush=True)
        return HTMLResponse(f"<pre>Error owner wallet: {_esc(str(e))}</pre>", status_code=500)


@app.get("/panel/rfc-owner/asignar-clon", response_class=HTMLResponse)
def panel_rfc_owner_asignar_clon(request: Request):
    return HTMLResponse("Acción no permitida. El saldo RFC se controla por bot interno desde Control por bot.", status_code=403)


@app.get("/panel/rfc-owner-status-fragment", response_class=HTMLResponse)
def panel_rfc_owner_status_fragment(request: Request):
    if not _is_valid_admin_panel_token(request):
        return HTMLResponse("No autorizado", status_code=403)
    ...
    try:
        ...
        if not row:
            return HTMLResponse("""
            <div class="box">
              <div class="head"><strong>💳 Saldo RFC del panel</strong></div>
              <div style="padding:16px;">Sin saldo configurado.</div>
            </div>
            """)

        now = datetime.now(timezone.utc)

        exp = row.get("idcif_expires_at")
        idcif_status = "INACTIVO"

        if row.get("idcif_enabled") and exp:
            if exp.tzinfo is None:
                exp = exp.replace(tzinfo=timezone.utc)
        ...
        exp_txt = "-"
        if row.get("idcif_expires_at"):
            try:
                exp_txt = row["idcif_expires_at"].strftime("%Y-%m-%d %H:%M")
            except Exception:
                exp_txt = str(row["idcif_expires_at"])
        ...
        <div class="box">
          <div class="head">
            <strong>💳 Saldo RFC del panel</strong>
            <span class="small">Saldo global asignado por proveedor. Solo lectura para el gestor.</span>
          </div>
          ...
          <div class="cards" style="padding:16px;">
            ...
            <div class="card">
              <div class="label">RFC CLON usados</div>
              <div class="value">{clon_used}</div>
            </div>

            <div class="card">
              <div class="label">IDCIF semanal</div>
              <div class="value" style="font-size:1.2rem;">{_esc(idcif_status)}</div>
            </div>

            <div class="card">
              <div class="label">Vence IDCIF</div>
              <div class="value" style="font-size:1rem;">{_esc(exp_txt)}</div>
            </div>

            <div class="card">
              <div class="label">Usos IDCIF</div>
              <div class="value">{idcif_used}</div>
            </div>
          </div>
        </div>
        """
        ...
        print("panel_rfc_owner_status_fragment error:", repr(e), flush=True)
        return HTMLResponse(f"<pre>Error saldo RFC: {_esc(str(e))}</pre>", status_code=500)
            ...
            if action == "set_limit":
                if family == "clon":
                    wallet = conn.execute(text("""
                        ...
                        FROM rfc_owner_wallets
                        WHERE owner_instance = 'grupo02'
                        LIMIT 1
                    """)).mappings().first()

                    global_available = int((wallet or {}).get("clon_balance") or 0)
                    ...
                    current_limit = int((current or {}).get("clon_limit") or 0)
                    current_used = int((current or {}).get("clon_used") or 0)
                    new_limit = max(value, 0)

                    current_available = 0 if current_limit == 0 else max(current_limit - current_used, 0)
                    new_available = 0 if new_limit == 0 else max(new_limit - current_used, 0)
                    extra_needed = max(new_available - current_available, 0)

                    if extra_needed > global_available:
                        return HTMLResponse(
                            f"No puedes asignar ese límite CLON. Disponible global: {global_available}. "
                            f"Aumento solicitado: {extra_needed}.",
                            status_code=400
                        )

                    conn.execute(text("""
                        UPDATE bot_control
                        SET clon_limit = :value, updated_at = now()
                        WHERE instance_name = :instance
                ...
                if family == "clon":
                    ...
                    if value > global_available:
                        return HTMLResponse(
                            f"No puedes recargar {value} RFC CLON. Disponible global del panel: {global_available}.",
                            status_code=400
                        )
...
# =========================================================
# RFC COMMUNICATION TOOLS HORIZONTAL V3
# Corrige cuando el bloque grande se fue completo a la izquierda.
# =========================================================

@app.get("/panel/communication-layout-v3.js")
def panel_communication_layout_v3():
    return Response("""
(function() {
  function textOf(el) {
  ...
  function preciseContainer(titleNode, ownText, otherText) {
    if (!titleNode) return null;
    ...
    for (let i = 0; i < 15 && cur; i++) {
      ...
      if (
        looksLikeBox &&
        txt.includes(ownText) &&
        !txt.includes(otherText) &&
        txt.length > 80
      ) {
        return cur;
      }
    ...
    // Fallback: agarrar el contenedor más pequeño posible aunque contenga menos texto.
    cur = titleNode;
    for (let i = 0; i < 8 && cur; i++) {
      const txt = textOf(cur);
      if (txt.includes(ownText) && !txt.includes(otherText)) {
        return cur;
      }
      cur = cur.parentElement;
    }
  ...
  function ensureLayout() {
    let finalBox = document.getElementById("rfcCommunicationToolsFinal");

    if (!finalBox) {
      finalBox = document.createElement("div");
      finalBox.id = "rfcCommunicationToolsFinal";
    ...
    let grid = finalBox.querySelector(".comm-grid");
    if (!grid) {
      grid = document.createElement("div");
      grid.className = "comm-grid";
      finalBox.appendChild(grid);
    }

    let massMount = document.getElementById("rfcMassMessagesFinalMount");
    let privateMount = document.getElementById("rfcPrivateMiniManagersFinalMount");

    if (!massMount || !privateMount) return null;

    return { finalBox, grid, massMount, privateMount };
  }

  function applyStyles() {
    let style = document.getElementById("rfcCommHorizontalV3Style");
    if (style) return;

    style = document.createElement("style");
    style.id = "rfcCommHorizontalV3Style";
    style.textContent = `
      #rfcCommunicationToolsFinal {
  ...
  function moveTools() {
    try {
      ...
      const massTitle = findTitle("Mensajes masivos");
      const privateTitle = findTitle("Mensaje privado a mini gestores RFC");

      const massBlock = preciseContainer(
        massTitle,
        "Mensajes masivos",
        "Mensaje privado a mini gestores RFC"
      );

      const privateBlock = preciseContainer(
        privateTitle,
        "Mensaje privado a mini gestores RFC",
        "Mensajes masivos"
      );

      if (massBlock && !massBlock.closest("#rfcCommunicationToolsFinal")) {
        layout.massMount.innerHTML = "";
        layout.massMount.appendChild(massBlock);
      }

      if (privateBlock && !privateBlock.closest("#rfcCommunicationToolsFinal")) {
        layout.privateMount.innerHTML = "";
        layout.privateMount.appendChild(privateBlock);
      }

      layout.finalBox.style.display = "";

    } catch(e) {
      console.error("RFC_COMMUNICATION_HORIZONTAL_V3_ERROR", e);
    }
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", moveTools);
  } else {
    moveTools();
  }

  setTimeout(moveTools, 500);
  setTimeout(moveTools, 1500);
  setTimeout(moveTools, 3000);
})();
